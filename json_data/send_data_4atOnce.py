...
for name, xlsx in flight_paths.items():
    df = (pd.read_excel(os.path.join(JSON_DIR, xlsx), sheet_name="in")
...
while active_flights:
    for flight_name in list(active_flights):  # copy to avoid modifying during iteration
        try:
        ...
        dist_m = pt.distance(nearest) * 111000  # rough conversion degrees to meters
        dist_ft = dist_m * 3.28084
        packet["deviation"] = round(dist_ft, 2)

        # send data
        resp = requests.post(URL, headers=HEADERS, json=packet)
        print(f"[{flight_name}] Sent {fname} → deviation {packet['deviation']} ft | HTTP {resp.status_code}")
